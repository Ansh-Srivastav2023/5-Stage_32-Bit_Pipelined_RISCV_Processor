module Decompressor (
    ...
    localparam OPCODE_SYSTEM = 7'b1110011; // Added for EBREAK
    ...
    // Registers in the x8-x15 range (C.Quadrant 1 prime registers)
    ...
    always @(*) begin
        ...
        final_funct3 = 3'b0;
        final_funct7 = 7'b0;
        final_opcode = 7'b0; // Defaults to illegal
        ...
        case ({funct3, op})
            ...
            {3'b001, 2'b01}: begin // C.JAL (RV32)
                imm32        = {{20{c_instr[12]}}, c_instr[12], c_instr[8], c_instr[10:9], c_instr[6], c_instr[7], c_instr[2], c_instr[11], c_instr[5:3], 1'b0};
                final_rd     = 5'd1; // ra
            ...
            {3'b100, 2'b01}: begin
                case (c_instr[11:10])
                    ...
                    2'b01: begin // C.SRAI
                        // FIX #1: SRAI needs bit 30 (imm[10]) set to 1 for arithmetic shift
                        imm32        = {20'b0, 7'b0100000, c_instr[6:2]}; 
                        final_rs1    = rs1_prime;
                        final_rd     = rd_prime;
                        final_funct3 = 3'b101; 
                        final_opcode = OPCODE_OP_IMM;
                    ...
                    2'b11: begin // Register ops (SUB, XOR, OR, AND)
                        final_rs1 = rs1_prime;
                        final_rs2 = rs2_prime;
                        final_rd  = rd_prime;
                        final_opcode = OPCODE_OP;
                        case ({c_instr[12], c_instr[6:5]})
                            // FIX #2: SUB requires funct7 to be 0100000
                            3'b000: {final_funct7, final_funct3} = {7'b0100000, 3'b000}; // SUB
                            ...
                            default: begin
                                {final_funct7, final_funct3} = {7'b0000000, 3'b000};
                            end
                        endcase
                    end
                endcase
            ...
            {3'b100, 2'b10}: begin
                if (c_instr[12] == 1'b0) begin
                    if (rs2 == 5'd0) begin // C.JR
                        ...
                        final_rs2    = rs2;
                        final_rd     = rd;
                        final_opcode = OPCODE_OP; // ADD rd, x0, rs2
                    end
                end else begin
                    if (rs2 == 5'd0) begin 
                         if (rd == 5'd0) begin // FIX #3: C.EBREAK handling
                             imm32        = 32'h00100073; // EBREAK instruction directly
                             final_opcode = OPCODE_SYSTEM; 
            ...
            default: begin
                final_opcode = 7'b0000000;
            end
        endcase

        // --- Final Assembly ---
        case(final_opcode)
            OPCODE_OP:     r_instr = {final_funct7, final_rs2, final_rs1, final_funct3, final_rd, final_opcode};
            OPCODE_OP_IMM: r_instr = {imm32[11:0], final_rs1, final_funct3, final_rd, final_opcode};
            OPCODE_LOAD:   r_instr = {imm32[11:0], final_rs1, final_funct3, final_rd, final_opcode};
            OPCODE_STORE:  r_instr = {imm32[11:5], final_rs2, final_rs1, final_funct3, imm32[4:0], final_opcode};
            OPCODE_BRANCH: r_instr = {{imm32[12],imm32[10:5]}, final_rs2, final_rs1, final_funct3, {imm32[4:1],imm32[11]}, final_opcode};
            OPCODE_LUI:    r_instr = {imm32[31:12], final_rd, final_opcode};
            OPCODE_AUIPC:  r_instr = {imm32[31:12], final_rd, final_opcode};
            OPCODE_JAL:    r_instr = {{imm32[20],imm32[10:1],imm32[11],imm32[19:12]}, final_rd, final_opcode};
            OPCODE_JALR:   r_instr = {imm32[11:0], final_rs1, final_funct3, final_rd, final_opcode};
            OPCODE_SYSTEM: r_instr = 32'h00100073; // Return standard EBREAK
            default:       r_instr = 32'b0;
        endcase
    end
endmodule